...
function* getUser (context, options) {
    ...
    if (options.strongToken && user.permanent) {
        throw new Error('This operation requires non-permanent JWT');
    }
...
function parseUrlPath (urlPath) {
    ...
    // URIdecode each command
    for (var i = 0; i < commands.length; i++) {
        commands[i] = decodeURIComponent(commands[i]);
    }
